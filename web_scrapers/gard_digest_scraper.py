...
# Broader heading tags to handle any restructuring of Gard's page
...
def _find_main_content_container(soup: BeautifulSoup) -> Tag:
    """
    Try a broad set of selectors before falling back to soup.body.
    """
    candidates = [
        "main",
        "article",
...
def _fallback_extract(container: Tag, base_url: str, article_title: str) -> List[Dict]:
    """
    Fallback: if the structured date-section parse finds nothing, collect every
    anchor inside the container that looks like a regulatory/external link.
    Groups them under the nearest preceding heading text.
    """
    items: List[Dict] = []
    seen_urls: set = set()

    current_heading = article_title
    for el in container.find_all(True):  # iterate all tags in document order
...
def discover_gard_digest_items(base_url: str) -> List[Dict]:
    ...
    - publisher
    - jurisdiction
    - _debug_headings  (only on first item, stripped by ingest)
    """
    ...
    # Collect all heading/paragraph elements — now includes h1, h4, h5, h6
    elements = container.find_all(list(_HEADING_TAGS) + ["p"], recursive=True)

    # Gather headings for debug output
    debug_headings = [
    ...
    # If structured parse found nothing, fall back to link extraction
    if not deduped:
        print(
            f"[GARD] Structured parse found 0 items. "
            f"Page headings seen: {debug_headings[:20]}. "
            f"Running fallback link extraction."
        )
        deduped = _fallback_extract(container, base_url, article_title)

    # Attach debug headings to first item so app.py can surface them
    if deduped:
